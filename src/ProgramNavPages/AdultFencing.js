...
function AdultProgramsSection() {
  ...
  }, [sectionData]); // precompute once [3]

  const handleCardClick = (href) => window.open(href, "_blank");
  ...
  const renderChooseTitle = (title) =>
    title.includes("Adult Path") ? (
      <>
        {title.split("Adult Path")}
  ...
  const ProgramCard = ({ program, index }) => (
    <div
      ...
      className={`
        ...
        p-5 sm:p-6 hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.01]
    ...
    >
      ...
      {/* Desktop hover schedule overlay */}
      <div className="hidden md:block absolute inset-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <h5 className="text-amber-700 font-semibold text-center mb-2 text-sm">SCHEDULE & PRICING</h5>
        <div className="text-center mb-3">
