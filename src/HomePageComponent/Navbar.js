...
const Navbar = () => {
  const [activeIndex, setActiveIndex] = useState(0); // Default to Home (now index 0)
  ...
  useEffect(() => {
    // Updated activeIndex mapping for new order:
    // 0: Home, 1: Programs, 2: Parents, 3: Store, 4: Summer Camps 2025
    if (location.pathname === "/store") {
      setActiveIndex(3); // Store is now at index 3
    } else if (location.pathname === "/") {
      setActiveIndex(0); // Home is now at index 0
    } else if (location.pathname === "/summercamp") {
      setActiveIndex(4); // Summer Camps is now at index 4
    } else if (location.pathname === "/founder") {
      setActiveIndex(0); // Keep Home active since it's in Home dropdown
    }
  }, [location.pathname]);
  ...
  const handleLogoClick = () => {
    navigate("/");
    setActiveIndex(0); // Updated to index 0 for Home
    setOpenDropdown(null);
    setIsMobileMenuOpen(false);
    setTimeout(() => {
      window.scrollTo(0, 0);
