...
export default function HeroSection({ slug = 'landing-page' }) {
  ...
  /* fetch hero once slug is known */
  useEffect(() => {
    sanityClient.fetch(HERO_BY_SLUG, { slug }).then(setData);
  }, [slug]);

  /* pause animations while user resizes */
  useEffect(() => {
    let t;
  ...
  const runSecondary = () => {
    ...
    if (a.startsWith('scroll:')) {
      ...
      window.location.href = a;      // internal path
    } else {
      ...
      <div className="absolute inset-0">
        {data.bgMob ? (
          <picture>
            ...
            <img src={data.bg} alt={data.bgAlt} className="w-full h-full object-cover animate-fade-in" />
          </picture>
        ) : (
          <img src={data.bg} alt={data.bgAlt} className="w-full h-full object-cover animate-fade-in" />
        )}
